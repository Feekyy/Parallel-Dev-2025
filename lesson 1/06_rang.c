...
{
    int n = 10;
    int input[10] = {5, 2, 8, 1, 9, 3, 7, 4, 6, 0};
    int output[10] = {0};
    
    char *source_str = kernelInput("06_kern.cl");
    ...
    cl_kernel kernel = clCreateKernel(program, "count_lower", &ret);
    if(ret != CL_SUCCESS)
    {
    ...
    printf("Rank (kisebb elemek száma): ");
    for (int i = 0; i < n; i++) 
    {
        printf("%d ", output[i]);
    }
