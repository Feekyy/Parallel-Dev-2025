...
{
    int n = 10;
    int input[10] = {5, 2, 8, 1, 9, 1, 7, 1, 6, 0};
    int output[10] = {0};
    
    char *source_str = kernelInput("07_kern.cl");
    ...
    cl_kernel kernel = clCreateKernel(program, "multi_member", &ret);
    if(ret != CL_SUCCESS)
    {
    ...
    printf("Elements count in the array: ");
    for (int i = 0; i < n; i++) 
    {
        printf("%d ", output[i]);
    }
